...
cv2.drawContours(copied1, contour2, -1, (255, 0, 0), 8)
for c in contour2:
    for p in c:
        cv2.circle(copied1, tuple(p[0]), 2, (0, 0, 255), -1)
cv2.putText(copied1, f'CHAIN_APPROX_SIMPLE, Number of contours : {len(contour2)}',
            (0, 10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1)
...
for i, c in enumerate(contour3):
    ...
    cv2.drawContours(copied2, contour3, i, color, 8)
    cv2.putText(copied2, str(i), tuple(c[0][0]), cv2.FONT_HERSHEY_PLAIN,
                1.5, (0, 0, 255), 2)
cv2.putText(copied2, f'TREE HiERARCHY & CHAIN_APPROX_SIMPLE, Number of contours : {len(contour2)}',
            (0, 10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1)


merged = np.vstack((img, copied1, copied2))
